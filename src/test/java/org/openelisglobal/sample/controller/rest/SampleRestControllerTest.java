...
public class SampleRestControllerTest extends BaseWebContextSensitiveTest {
    ...
    //
    // Blocked by SampleDAOImpl HQL bug: query uses "r.canceled = false" but
    // Referral
    // has no canceled property (uses ReferralStatus enum). This is unrelated to
    // #3642/#3701, which fixed SampleTypeRequestDAOImpl binding sampleId as Integer
    // instead of String for a different code path.
    //
    // Uncomment these tests once SampleDAOImpl.getUnassignedSampleByAccessionNumber
    // is aligned with ReferralDAOImpl (e.g. r.status != 'CANCELED').
    ...
    // throws Exception {
    // MvcResult mvcResult =
    // super.mockMvc.perform(get("/rest/sample/unassigned-by-accession/654321")
    ...
    //
    // String content = mvcResult.getResponse().getContentAsString();
    // List<SampleSearchForm> forms = Arrays.asList(super.mapFromJson(content,
    // SampleSearchForm[].class));
    //
    // assertEquals(2, forms.size());
    // assertEquals(654321, Integer.parseInt(forms.get(0).getAccessionNumber()));
    // }
    ...
    // throws Exception {
    // MvcResult mvcResult =
    // super.mockMvc.perform(get("/rest/sample/unassigned-by-accession/NONEXISTENT")
    // .accept(MediaType.APPLICATION_JSON_VALUE)
    // .contentType(MediaType.APPLICATION_JSON_VALUE)).andReturn();
    //
    // assertEquals(404, mvcResult.getResponse().getStatus());
    ...
    // throws Exception {
    // MvcResult mvcResult =
    // super.mockMvc.perform(get("/rest/sample/unassigned-by-accession/123456")
    // .accept(MediaType.APPLICATION_JSON_VALUE)
    // .contentType(MediaType.APPLICATION_JSON_VALUE)).andReturn();
    //
    // assertEquals(404, mvcResult.getResponse().getStatus());
